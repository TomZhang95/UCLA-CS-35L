...
//Start point of threads

void* startingPointOfThreads(void *arg)
{
    struct myStruct* t_vars;
    t_vars = (struct myStruct*) arg;
    
    int startingPixel = t_vars->threadArgument;
    
    /* for every pixel */
    for( int px=startingPixel; px<width; px+=t_vars->nthreads )
    {
        const double x = t_vars->pixel_dx * ((double)( px-(width/2) ));
        for( int py=0; py<height; ++py )
        {
            const double y = t_vars->pixel_dy * ((double)( py-(height/2) ));
            Vec3 pixel_color;
            ...
            for( int xs=-halfSamples; xs<=halfSamples; ++xs )
            {
                ...
                {
                    double subx = x + ((double)xs)*t_vars->subsample_dx;
                    double suby = y + ((double)ys)*t_vars->subsample_dy;
                    
                    /* construct the ray coming out of the camera, through
                     * the screen at (subx,suby)
                     */
                    ray_t pixel_ray;
                    copy( pixel_ray.org, t_vars->camera_pos );
                    Vec3 pixel_target;
                    set( pixel_target, subx, suby, z );
                    sub( pixel_ray.dir, pixel_target, t_vars->camera_pos );
                    norm( pixel_ray.dir, pixel_ray.dir );
                    
                    Vec3 sample_color;
                    copy( sample_color, t_vars->bg_color );
                    /* trace the ray from the camera that
                     * passes through this pixel */
                    trace( &vars.scene, sample_color, &pixel_ray, 0 );
            ...
            t_vars->scaled_color[px][py][0] = gamma( pixel_color[0] ) * max_color;
            t_vars->scaled_color[px][py][1] = gamma( pixel_color[1] ) * max_color;
            t_vars->scaled_color[px][py][2] = gamma( pixel_color[2] ) * max_color;
            
            /* enforce caps, replace with real gamma */
            for( int i=0; i<3; i++)
                t_vars->scaled_color[px][py][i] = max( min(t_vars->scaled_color[px][py][i], 255), 0);
        }
    }
...
int main( int argc, char **argv )
{
    ...
    vars.nthreads = argc == 2 ? atoi( argv[1] ) : 0;
    ...
    set( vars.camera_pos, 0., 0., -4. );
    set( vars.camera_dir, 0., 0., 1. );
    set( vars.bg_color, 0.8, 0.8, 1 );
    vars.camera_fov = 75.0 * (PI/180.0);
    vars.pixel_dx = tan( 0.5*vars.camera_fov ) / ((double)width*0.5);
    vars.pixel_dy = tan( 0.5*vars.camera_fov ) / ((double)height*0.5);
    vars.subsample_dx =
    halfSamples ? vars.pixel_dx / ((double)halfSamples*2.0) : vars.pixel_dx;
    vars.subsample_dy =
    halfSamples ? vars.pixel_dy / ((double)halfSamples*2.0) : vars.pixel_dy;
    
    
    pthread_t* thread = malloc(sizeof(pthread_t) * vars.nthreads);
    if(thread == NULL)
        fprintf(stderr, "Thread Malloc faied");
    
    /* Write the image format header */
    /* P3 is an ASCII-formatted, color, PPM file */
    vars.scene = create_sphereflake_scene( sphereflake_recursion );
    printf( "P3\n%d %d\n%d\n", width, height, max_color );
    printf( "# Rendering scene with %d spheres and %d lights\n",
           vars.scene.sphere_count,
           vars.scene.light_count );
    
    vars.threadArgument = 0;
    
    for (int i = 0; i < vars.nthreads; i++)
    {
    ...
    for (int i = 0; i < vars.nthreads; i++)
    {
        fail = pthread_create(&thread[i], NULL, startingPointOfThreads, &vars);
        if(fail)
            fprintf(stderr, "Creating threads error");
        vars.threadArgument++;
    }
    ...
    for(int px = 0; px < width; px++)
    {
        for(int py = 0; py < height; py++)
        {
             ...
             */
            printf( "%.0f %.0f %.0f\n", vars.scaled_color[px][py][0], vars.scaled_color[px][py][1], vars.scaled_color[px][py][2] );
